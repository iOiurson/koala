...
def dump(self, fname, marshal_found = False):
    ...
    def parse_cell_info(cell):
        ...
        # print 'Cell closure', cell.calculate.__closure__
        if cell.calculate is not None:
            function = marshal.dumps(cell.calculate.func_code)
            closure = str(cell.calculate.__closure__)
        else:
            function = "0"
            closure = "0"
...
def load(fname):
    ...
    for line in infile.read().splitlines():
        ...
        if mode == "node0":
            ...
            [address, formula, code, closure, is_range, is_named_range, is_volatile, should_eval] = line.split(SEP)

            formula = clean_bool(formula)
            # print 'CODE', code
            print 'CC', closure, type(closure)
            cc = marshal.loads(code)
            closure = make_tuple(closure) if closure != '0' else None
            print 'CC 2', type(closure)
            function = types.FunctionType(cc, globals(), closure = closure) if code != '0' else None
